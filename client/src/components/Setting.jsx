...
const ProfileInformation = ({ userId, user, onSave, onCancel, profilepic }) => {
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-gray-700 bg-opacity-50 flex justify-center items-center transition-opacity duration-300 z-9999">
          <div className="bg-black rounded-lg p-6 w-80 sm:w-96 z-9999">
            ...
            <input
              type="file"
              accept="image/png, image/jpg, image/jpeg, image/gif, image/webg"
...
const SkillInput = () => {
  ...
  const skillSchema = Joi.string()
    ...
    .messages({
      ...
      "string.pattern.base":
        "Skill must start with a letter and can contain letters, numbers, and special characters like !@#$%^&*().",
    });
  ...
  const fetchUser = async () => {
    try {
      ...
      if (isLogged) {
        ...
        setSkills(res.data.user.skills || []); // Fallback to empty array if no skills
      }
    } catch (err) {
      console.error("Error fetching user details:", err);
  ...
  const addSkill = async () => {
    ...
    try {
      ...
      showSnackbar(
        err.response?.data?.error || "Failed to add skill.",
        "error"
      );
    }
  };
  ...
  const removeSkill = async (skill) => {
    try {
      const response = await axios.post(
        "http://localhost:5000/api/skills/remove-skill",
        {
          sessionemail,
          skill,
        }
      ...
      showSnackbar(
        err.response?.data?.error || "Failed to remove skill.",
        "error"
      );
    }
  };
        ...
        <div className="space-y-2 mt-4 pb-1">
          {skills.length === 0 ? (
            ...
            skills.map((skill, index) => (
              ...
              >
                <span className="text-white">{skill.toUpperCase()}</span>{" "}
  ...
  const fetchUser = async () => {
    try {
      ...
      if (isLogged) {
        ...
        const { line1, state, city, pincode } = res.data.user;
        console.log(line1, state, city, pincode);
        setLine1(line1 || "");
        setState(state || "");
        setCity(city || "");
        setPincode(pincode || "");
...
const DangerZone = () => {
  ...
  ); // Assuming you store session email in localStorage
        ...
        <div className="w-3/4 max-w-3xl mx-auto py-12 px-4 lg:px-8">
          <SettingSection icon={User} title="Profile">
            <ProfileInformation
              ...
              profilepic={userDetail?.profilePhoto}
              user={sessionemail}
